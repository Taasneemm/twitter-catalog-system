...
def Register():
    if request.method == "POST":
        ...
        if user:
            ...
            new_user = fitwellUser(email = email, password = passwd, NRIC = nric)
            return render_template('index.html')
...
@app.route('/Catalog')
def Catalog():
    return render_template('catalog.html', name=current_user.record['email'])
...
def Dashboard():
    ...
    #print("Email logout session is (dashboard page): " )
...
def Upload():
    ...
    return render_template('upload.html', name=current_user.record['email'])
